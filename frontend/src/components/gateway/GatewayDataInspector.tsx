...
import { Layers, Hash, Cpu, Database } from "lucide-react";
...
export const GatewayDataInspector: React.FC<GatewayDataInspectorProps> = ({
  ...
  return (
    <div
      className="w-full rounded-2xl border p-4 sm:p-5 flex flex-col gap-4 font-mono text-xs transition-all shadow-sm"
      style={{
    ...
    >
      <div className="flex items-center justify-between pb-3 border-b" style={{ borderColor: "rgb(var(--border))" }}>
        <span className="text-[11px] font-bold uppercase text-caption tracking-wider">
          Real-Time Data Transformation Inspector
        </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-emerald-500 font-bold">SORTED</span>
          </div>

          <pre className="p-2.5 rounded-lg bg-slate-950 text-emerald-400 text-[11px] leading-relaxed overflow-x-auto max-h-28 border border-slate-800">
        ...
        >
          <div className="flex flex-col gap-1.5">
            ...
            <span className="p-2 rounded-lg bg-slate-950 text-sky-400 text-[11px] truncate border border-slate-800">
              {packet.contextHashHex}
            </span>
          </div>

          <div className="flex flex-col gap-1.5 pt-2 border-t" style={{ borderColor: "rgb(var(--border))" }}>
            <span className="text-[10px] text-caption uppercase font-semibold flex items-center gap-1">
              <Cpu className="w-3.5 h-3.5 text-sky-500" />
              <span>ML-DSA-65 Demonstration Provider Signature</span>
            </span>
            <span className="p-2 rounded-lg bg-slate-950 text-indigo-300 text-[11px] truncate border border-slate-800">
              {packet.signatureHex}
            </span>
          </div>
        </div>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-caption font-semibold">REPLAY STORE LOOKUP:</span>
